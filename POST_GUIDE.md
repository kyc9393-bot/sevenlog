...
**이미지 구하기 (무료 스톡 전용 — AI 이미지 생성 서비스 사용 금지)**

1. **Wikimedia Commons에서만 구합니다.** Higgsfield 등 AI 이미지 생성 도구는 크레딧이 소모되므로 사용하지 않습니다.
...
2. 주제와 맞는 사진을 찾기 어려운 소제목은 억지로 채우지 말고 이미지 수를 줄입니다(대표 1장 + 본문 0~3장). 4장을 못 채워도 발행에는 문제가 없습니다.
3. 스톡 검색이 전부 실패하면 이미지 없이 발행해도 됩니다(빌드 실패보다 낫습니다).
